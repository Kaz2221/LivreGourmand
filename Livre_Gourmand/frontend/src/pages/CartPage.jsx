...
const CartPage = () => {
  ...
  const calculateTotal = () => {
    ...
    return cart.Livres.reduce((total, book) => {
      const price = parseFloat(book.item_panier?.prix_unitaire || book.prix);
      const quantity = book.item_panier?.quantite || 1;
      ...
      <div className="container mx-auto px-4">
        ...
        {(!cart || !cart.Livres || cart.Livres.length === 0) ? (
          <div className="bg-white rounded-lg shadow-md p-6 text-center">
            ...
            <Link to="/shop" className="bg-primary text-white px-6 py-2 rounded-md inline-flex items-center">
                          ...
                          >
                            <td className="px-6 py-4">
                              <div className="flex items-center">
                                <div className="h-20 w-16 bg-gray-200 overflow-hidden rounded">
                                  {book.image_url ? (
                                    <img
                            ...
                            <td className="px-6 py-4">
                              <div className="flex items-center border rounded-md w-28">
                                <button onClick={() => handleQuantityChange(book.id_livre, quantity - 1)} disabled={quantity <= 1} className="px-2 py-1 text-gray-600 hover:text-primary disabled:text-gray-300">
                                  <FaMinus />
                                </button>
                                <span className="mx-2 w-8 text-center">{quantity}</span>
                                <button onClick={() => handleQuantityChange(book.id_livre, quantity + 1)} className="px-2 py-1 text-gray-600 hover:text-primary">
                                  <FaPlus />
                                </button>
                              </div>
                            </td>
                            <td className="px-6 py-4 font-medium text-gray-900">{itemTotal} €</td>
                            <td className="px-6 py-4 text-right">
                              <button onClick={() => handleRemoveItem(book.id_livre)} className="text-red-600 hover:text-red-800">
                                <FaTrash />
                              </button>
                            </td>
                          </motion.tr>
                ...
                <div className="border-t pt-4">
                  ...
                  <div className="border-t mt-4 pt-4">
                    <div className="flex justify-between mb-2">
                      ...
                      <span className="text-lg text-primary font-semibold">{(subtotal + 5).toFixed(2)} €</span>
                    </div>
                  </div>
                  <button onClick={handleCheckout} disabled={loading} className="mt-6 w-full bg-primary text-white py-2 px-4 rounded-md hover:bg-primary/90 transition-colors disabled:opacity-60">
                    Procéder au paiement
                  </button>
                  <Link to="/shop" className="mt-4 w-full block text-center text-primary hover:underline">
